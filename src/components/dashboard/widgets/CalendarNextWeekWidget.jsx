...
export default function CalendarNextWeekWidget() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const weekStart = startOfWeek(nextWeek, { weekStartsOn: 6 });
        const weekEnd = endOfWeek(nextWeek, { weekStartsOn: 6 });
        ...
        setEvents(nextWeekEvents.slice(0, 8));
      } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        <div className="text-sm text-slate-600">Next Week Events</div>
