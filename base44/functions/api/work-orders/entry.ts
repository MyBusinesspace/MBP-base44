  ...
  try {
    ...
    if (method === 'POST') {
      ...
      try {
        ...
        // Asignación del número: 0001/YY anclado a la fecha/hora de creación real (único)
        // Primero creamos el registro SIN número; luego numeramos usando created_date persistido

        // Set default values (without work_order_number)
        const workOrderData = {
          title: body.title || '',
          ...
          // work_order_number: se asigna tras la creación usando created_date
          task: body.task || '',
          archived: false,
          is_active: false
        };
        ...
        // Resolve branch (prefer entity field, fallback a project)
        let branchId = createdWorkOrder.branch_id || null;
        if (!branchId && createdWorkOrder.project_id) {
          try {
            const proj = await base44.asServiceRole.entities.Project.get(createdWorkOrder.project_id);
            branchId = proj?.branch_id || null;
          } catch {}
        }
        // Generar número único estrictamente desde created_date
        if (branchId) {
          const dateRef = createdWorkOrder.created_date || new Date().toISOString();
          let number = null;
          for (let i = 0; i < 6; i++) {
            const resNum = await base44.functions.invoke('getNextWorkOrderNumberAtomic', { branch_id: branchId, date: dateRef });
            const candidate = resNum?.data || null;
            if (!candidate) break;
            const clashes = await base44.asServiceRole.entities.TimeEntry.filter({ work_order_number: candidate }, '-updated_date', 1);
            if (!Array.isArray(clashes) || clashes.length === 0) { number = candidate; break; }
          }
          if (number) {
            await base44.asServiceRole.entities.TimeEntry.update(createdWorkOrder.id, { work_order_number: number });
            createdWorkOrder.work_order_number = number;
          }
        }
    ...
    if (method === 'PUT') {
      ...
      try {
        ...
        // Preserve immutable serial number; if missing or invalid, try to repair
        try {
          const existing = await base44.asServiceRole.entities.TimeEntry.get(workOrderId);
          const current = existing?.work_order_number;
          const isValid = typeof current === 'string' && /^\d{4}\/\d{2}$/.test(current);
          if (isValid) {
            updateData.work_order_number = current;
          } else {
            // attempt to repair
            let branchId = existing?.branch_id || null;
            if (!branchId && existing?.project_id) {
              try {
                const proj = await base44.asServiceRole.entities.Project.get(existing.project_id);
                branchId = proj?.branch_id || null;
              } catch {}
            }
            const dateRef = existing?.planned_start_time || existing?.created_date || new Date().toISOString();
            if (branchId) {
              for (let i = 0; i < 6; i++) {
                ...
                if (!candidate) break;
                const clashes = await base44.asServiceRole.entities.TimeEntry.filter({ work_order_number: candidate }, '-updated_date', 1);
                if (!Array.isArray(clashes) || clashes.length === 0) { updateData.work_order_number = candidate; break; }
              }
            }
          }
        } catch {}
