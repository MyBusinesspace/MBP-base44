...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch entries with service role (bypass user restrictions)
    ...
    for (const e of candidates) {
      ...
      try {
        ...
        if (!dry_run) {
          await base44.asServiceRole.entities.TimeEntry.update(e.id, { work_order_number: won, updated_by: 'service_backfill' });
        }
    ...
    return Response.json({ error: error?.message || 'Failed to force backfill WOs' }, { status: 500 });
  }
});