...
/**
 * Sends calendar event invitations via API
 * Used by apiCalendarEvents to send invitations when creating events
 */
Deno.serve(async (req) => {
  try {
    ...
    // ✅ Get current user info for email organizer
    let organizerName = 'MyBusinessPace Calendar';
    let organizerEmail = 'calendar@mybusinesspace.com';
    try {
      const users = await base44.asServiceRole.entities.User.list('sort_order', 1);
      const user = users[0];
      if (user) {
        organizerName = `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.email;
                        ...
                        <td style="padding: 0 30px 30px 30px;">
                            ...
                            <div style="margin-top: 25px; padding-top: 20px; border-top: 1px solid #e0e0e0;">
                                <p style="margin: 0; color: #999999; font-size: 13px;">
                                    <strong>Organized by:</strong> ${organizerName} (${organizerEmail})
                                </p>
                            </div>
                        </td>
                    </tr>
